...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 bg-(--surface) border-r-2 border-(--outline-variant) flex flex-col h-full font-sans justify-between select-none shrink-0">
          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              ...
              >
                ...
                <link.icon className={`w-4.5 h-4.5 ${isActive ? "text-primary" : "text-(--on-surface-variant)"}`} />
                {link.name}
              </Link>
            );
          })}
      ...
      <div className="p-4 border-t border-(--outline-variant) flex items-center gap-3">
        ...
        <div className="flex flex-col min-w-0">
          ...
          <span className="text-[10px] font-medium text-(--on-surface-variant) truncate mt-0.5">
            Master Studio
          </span>
        </div>
      </div>
