...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 bg-white border-r-2 border-gray-100 flex flex-col h-full font-sans justify-between select-none shrink-0">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3.5 pl-6 pr-6 py-3.5 text-sm font-semibold transition-all relative ${
                  isActive
                    ? "bg-[#E6F0FF] text-[#0066FF] rounded-none"
                    : "text-gray-500 hover:bg-gray-50 hover:text-gray-900"
                }`}
              >
                {isActive && (
                  <div className="absolute right-0 top-0 bottom-0 w-[4px] bg-[#0066FF]"></div>
                )}
                <link.icon className={`w-[18px] h-[18px] ${isActive ? "text-[#0066FF]" : "text-gray-500"}`} />
                {link.name}
              </Link>
            );
          })}
