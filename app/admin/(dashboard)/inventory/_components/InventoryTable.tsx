...
import { CheckCircle2, AlertTriangle, Info, MoreVertical } from "lucide-react";
...
export function InventoryTable() {
  ...
  const columns: ColumnDef<InventoryItem>[] = useMemo(
    () => [
      {
        accessorKey: "model",
        header: () => <div className="font-bold text-gray-700 py-2">SHOE MODEL</div>,
        cell: ({ row }) => {
      ...
      {
        accessorKey: "sku",
        header: () => <div className="font-bold text-gray-700 py-2">SKU</div>,
        cell: ({ row }) => {
      ...
      },
      {
        ...
        header: () => <div className="font-bold text-gray-700 py-2">CATEGORY</div>,
        cell: ({ row }) => {
      ...
      {
        accessorKey: "price",
        header: () => <div className="font-bold text-gray-700 py-2">PRICE</div>,
        cell: ({ row }) => {
      ...
      },
      {
        ...
        header: () => <div className="font-bold text-gray-700 py-2">STOCK LEVEL</div>,
        cell: ({ row }) => {
          const item = row.original;
      ...
      {
        id: "actions",
        header: () => <div className="font-bold text-gray-700 text-right py-2"></div>,
        cell: () => {
          return (
            <div className="flex justify-end pr-2">
              <SubmitButton type="button" className="text-gray-400 hover:text-gray-600 p-1 transition-colors bg-transparent border-0 shadow-none">
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mt-8">
      ...
      <div className="p-6 bg-[#F8FAFC] flex items-center justify-between border-t border-gray-100">
        <p className="text-sm text-gray-500 font-medium">Showing {data.length} of 1,284 products</p>
        <div className="flex items-center gap-3">
          <SubmitButton type="button" className="px-4 py-2 text-sm font-medium text-gray-600 bg-transparent border border-gray-200 rounded-md hover:bg-gray-50 transition-colors">
            Previous
          </SubmitButton>
          <SubmitButton type="button" className="px-4 py-2 text-sm font-medium text-white bg-[#0A3D91] hover:bg-[#082f70] rounded-md transition-colors shadow-sm">
            Next
          </SubmitButton>
        </div>
      </div>
